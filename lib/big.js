...
window.addEventListener('load', function() {
  ...
  function onKeyDown(e) {
    ...
    case "ArrowUp":
    case "PageUp":
      if (big.mode === "talk") { reverse(); return; }
      break;
    case "ArrowRight":
    case "ArrowDown":
    case "PageDown":
      if (big.mode === "talk") { forward(); return; }
      break;
    case "p":
      onPrint();
      break;
    ...
    if (big.mode === "jump" && document.activeElement) {
      var current = document.activeElement.getBoundingClientRect();
      var column = [];
      for (var i = 0; i < slideDivs.length; i++) {
        var rect = slideDivs[i].firstChild.getBoundingClientRect();
        if (rect.left === current.left) {
          column.push(slideDivs[i].firstChild);
        }
      }
      switch (e.key) {
      case "ArrowLeft":
        slideDivs[slideDivs.indexOf(document.activeElement.parentNode) - 1].firstChild.focus();
        break;
      case "ArrowRight":
        slideDivs[slideDivs.indexOf(document.activeElement.parentNode) + 1].firstChild.focus();
        break;
      case "ArrowDown":
        column[column.indexOf(document.activeElement) + 1].focus();
        break;
      case "ArrowUp":
        column[column.indexOf(document.activeElement) - 1].focus();
        break;
